...
def download_wat_paths(CC_archive_name):
    ...
    local_file = os.path.join(DIRECTORY, "wat.paths.gz")
    ...
    SOURCE_DIRECTORY = "wat_paths_gz"
    DESTINATION_DIRECTORY = "wat_paths"
    DESTINATION_FILE =f"wat_paths_{CC_archive_name}.txt"
    os.makedirs(DESTINATION_DIRECTORY, exist_ok=True)

    print(INFO + f"Lecture du fichier {local_file}")

    with gzip.open(local_file, "rb") as fgz:
        wat_paths = [line.decode("utf-8").strip() for line in fgz]
